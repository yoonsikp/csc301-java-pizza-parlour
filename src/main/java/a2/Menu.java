 ...
 * A Menu contains all items on the menu in the restaurant and includes their options and prices.
 */
public class Menu {
    ...
    private String getPizzaSizesAsString(String type) {
        ...
        for (String size : this.pizzaSet.get(type).keySet()) {
            menuString.append(size).append(" ($").append(this.pizzaSet.get(type).get(size).toString()).append(") ");
        }
