...
/**
 * This is the TerminalInterface class. The TerminalInterface is used to simulate/facilitate a User
 * Interface through the command line, and to facilitate the interactions between the command line
 * UI and the back-end restaurant process. The TerminalInterface class has multiple constructors in
 * order to maintain low constructor tunneling.
 */
class TerminalInterface {
  ...
  /**
   * Constructor 3 calls constructor 4 with the desired input stream, pizza parlour's menu, and
   * handlers.
   *
   * @param inputStream input stream to read from
  ...
  /**
   * Constructor 4 sets the desired input stream, pizza parlour's menu, and handlers for the
   * TerminalInterface.
   *
   * @param inputStream input stream to read from
   * @param menu menu for pizza parlour
  ...
  /**
   * Starts reading from the command line. Available commands are determined by the state of the
   * TerminalReader. State is determined by attributes currentOrder and currentFood, which are set
   * in terms of whether or not an order has been created and is currently in use and whether or not
   * any food items have been added to the order. It first welcomes user to program, and then is
   * able to print a list of all possible commands at each state. Depending on the commands chosen,
   * it redirects user to other methods that enact functionalities of the commands.
   */
  void startReading() {
    // Input
    String genInput;
  ...
  /**
   * This method handles the provided command depending on the state of the TerminalInterface. The
   * states are determined by whether or not there is a current order.
   *
  ...
  /**
   * This method handles the provided command depending on the state of the TerminalInterface. The
   * states are determined by whether or not there is a current food in the current order.
   *
   * @param command the command provided in the UI
   */
  ...
  /**
   * This method handles adding a new Drink to the order. It asks the user for the drink type and
   * then facilitates creation of a new Drink.
   *
  ...
  /**
   * This method handles adding a new Pizza to the order. It asks the user for the Pizza type, size
   * and toppings and then facilitates creation of a new Pizza.
   *
  ...
  /**
   * This method facilitates printing to the screen details of the delivery associated with the
   * current order as JSON, CSV or text depending on the delivery type.
   */
  ...
  /**
   * This method prints all options in a given list with associated numbers so that the user can
   * choose which option they want.
   *
