...
public class TerminalInterfaceTest {
  ...
  public void basicConstructorTest() {
    ...
    assertNotEquals(testTerm.hashCode(), 0);
  }
  ...
  public void dishHelp() {
    InputStream stream = new ByteArrayInputStream(
        "neworder\nnewdrink\n0\nseldish\n0\n?\nexit\n".getBytes(StandardCharsets.UTF_8));
    TerminalInterface testTerm = new TerminalInterface(stream, getFakeMenu(), new OrderHandler(),
        new DeliveryHandler());
    testTerm.startReading();
    assertEquals("\trmdish            \tRemove the Current Dish from the Order",
        outContent.toString().split("\\n")[outContent.toString().split("\\n").length - 3]);
    assertEquals("\t..                \tDeselect Currently Selected Dish",
        outContent.toString().split("\\n")[outContent.toString().split("\\n").length - 2]);
  }
  ...
  private Menu getFakeMenu() {
    Menu menu = new Menu();
    HashMap<String, Float> testDrinks = new HashMap<>();
